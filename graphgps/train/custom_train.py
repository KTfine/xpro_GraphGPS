...
def log_flagpe_parameters(model, epoch, run_dir):
    """
    Log FLaGPE alpha and beta parameters if using SANTransformer with FLaGPE.
    ...
    """
    # Check if model uses SANTransformer and FLaGPE is enabled
    if not (hasattr(cfg, 'posenc_FLaGPE') and cfg.posenc_FLaGPE.enable):
        return
    
    flagpe_params = {}
    
    # Extract alpha and beta from SANLayers
    if hasattr(model, 'model') and hasattr(model.model, 'trf_layers'):
        # SANTransformer case
        for layer_idx, layer in enumerate(model.model.trf_layers):
            if hasattr(layer, 'alpha') and hasattr(layer, 'beta'):
                alpha_val = torch.sigmoid(layer.alpha).item()
                beta_val = torch.sigmoid(layer.beta).item()
                flagpe_params[f'layer_{layer_idx}'] = {
                    'alpha': round(alpha_val, 6),
                    'beta': round(beta_val, 6)
                }
    
    if not flagpe_params:
        return
    
    # Log to console
    logging.info(f"Epoch {epoch} - FLaGPE Parameters:")
    for layer_name, params in flagpe_params.items():
        logging.info(f"  {layer_name}: alpha={params['alpha']:.4f}, beta={params['beta']:.4f}")
    
    # Save to JSON file
    params_dir = os.path.join(run_dir, 'flagpe_params')
    os.makedirs(params_dir, exist_ok=True)
    
    params_file = os.path.join(params_dir, f'epoch_{epoch:04d}.json')
    with open(params_file, 'w') as f:
        json.dump(flagpe_params, f, indent=2)
    
    # Also maintain a CSV for easy plotting
    csv_file = os.path.join(params_dir, 'flagpe_params.csv')
    header = False
    ...
    with open(csv_file, 'a') as f:
        if header:
            # Write header
            max_layers = len(flagpe_params)
            header_row = ['epoch'] + [f'layer_{i}_alpha' for i in range(max_layers)] + \
                         [f'layer_{i}_beta' for i in range(max_layers)]
            f.write(','.join(header_row) + '\n')
        
        # Write data row
        row = [str(epoch)]
        for layer_name in sorted(flagpe_params.keys()):
            row.append(str(flagpe_params[layer_name]['alpha']))
        for layer_name in sorted(flagpe_params.keys()):
            row.append(str(flagpe_params[layer_name]['beta']))
        f.write(','.join(row) + '\n')
...
def custom_train(loggers, loaders, model, optimizer, scheduler):
    ...
    for cur_epoch in range(start_epoch, cfg.optim.max_epoch):
        ...
        # Log FLaGPE alpha and beta parameters if enabled
        if hasattr(cfg, 'posenc_FLaGPE') and cfg.posenc_FLaGPE.enable:
            log_flagpe_parameters(model, cur_epoch, cfg.run_dir)
